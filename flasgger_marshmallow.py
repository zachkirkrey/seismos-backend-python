        ...
        def parse_simple_schema(c_schema, location):
            ...
            for key, value in c_schema.__dict__.get("_declared_fields").items():
                ...
                if not values_real_types:
                    raise "Unsuported type: %s" % str(type(value))
                tmp = {
                    "in": location,
        ...
        def wrapper(*args, **kw):
            ...
            try:
                path_schema and setattr(
                    request, "path_schema", path_schema().load(path_params or {})
                )
                query_schema and setattr(
                    request, "query_schema", query_schema().load(query_params or {})
                )
                form_schema and setattr(
                    request, "form_schema", form_schema().load(form_params or {})
                )
                json_schema and setattr(
                    request, "json_schema", json_schema().load(json_params or {})
                )
                headers_schema and setattr(
                    request,
                    "headers_schema",
                    headers_schema().load(dict(header_params)),
                )
